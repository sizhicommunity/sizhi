import { getStringFromUrl, sSProxyOptions } from "../get";
...
export class StringSource implements Source {
  ...
  getFeedInfo(): FeedInfo {
    return { defineUrl: "mockString", feedPath: "mockString" ,url:'mockFeedUrl'};
  }
}
export class HttpSource implements Source {
  ...
  getString(): Promise<string> {
    try{
    return getStringFromUrl(this.url,sSProxyOptions);
    }catch(e){
      console.log(e)
